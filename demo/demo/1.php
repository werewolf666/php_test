...
Class Model_Member_Order extends ORM
{
     ...
     * 订单来源
     */
    public static $order_source = array(
        '1' => '电脑端',
        '2' => '手机端',
        '3' => '小程序',
        '4' => '平台添加'
     ...
     * @return string
     */
    public static function get_status_name($status,$typeid=0)
    {
        if($typeid!=1)
        {
    ...
    public static function order_info($ordersn, $memberid = null)
    {
        ...
        {
            $rs['envelope_price'] = Model_Order_Envelope::get_order_envelope_price($ordersn);
        }
        ...
        $rs['total'] = $rs['payprice'];
        $rs['pay_price'] = $rs['payprice'];
        //订单总价信息
        $total_info = Model_Member_Order_Compute::get_order_price($rs['id']);
        if($total_info && ($total_info['total_price'] > 0))
        {
            //订单总价
            $rs['totalprice'] = $total_info['total_price'];
    ...
    /**
     * @function 通过ordersn获取关联订单总价
     * @param $ordersn
     * @return float $pay_total
     */
    public static function cur_order_total_pay_price($ordersn){
        ...
        if (!is_null($ordersn))
        {
            ...
            if (St_Functions::is_normal_app_install('red_envelope')) {
                ...
                $envelope_price = Model_Order_Envelope::get_order_envelope_price($ordersn);
    ...
    public static function order_total_payprice($orderid, $orderInfo = null)
    {
        ...
        if (St_Functions::is_normal_app_install('red_envelope')) {
            ...
            $envelope_price = Model_Order_Envelope::get_order_envelope_price($orderInfo['ordersn']);
    ...
    public static function order_privileg_price($orderid, $orderInfo = null)
    {
        ...
        //平台内部优惠
        if($orderInfo['platform_discount'])
        {
            $privileg_price += $orderInfo['platform_discount'];
        }
        //红包优惠
        if(St_Functions::is_normal_app_install('red_envelope'))
        {
            $envelope_price = Model_Order_Envelope::get_order_envelope_price($orderInfo['ordersn']);
            $privileg_price += $envelope_price;
...
sql;
    ...
    public static function back_order_list($params)
    {
        ...
        if (!empty($keyword)) {
            $w .= " and (a.ordersn like '%{$keyword}%' or a.linkman like '%{$keyword}%' or a.linktel like '%{$keyword}%' or a.linkemail like '%{$keyword}%' or a.productname like '%{$keyword}%' or a.eticketno like'%{$keyword}%')";
        }
        /* if ($webid >= 0) {
             $w .= " and a.webid=" . $webid;
         }*/
    ...
    public static function back_order_status_changed($org_status, $order, $product_model)
    {
        ...
        if ($order['status'] == 2)
        {
            if($org_status==0 || $org_status==1 || $org_status == 3)
            {
            ...
            if (St_Functions::is_normal_app_install('red_envelope'))
            {
                Model_Order_Envelope::add_order_envelope($order['ordersn'],$order);
            }
        }
        //取消订单(加库存)
        if ($order['status'] == 3 && $org_status != 4)
        {
            ...
            self::discounts_cancel($order['ordersn'],$order);//处理优惠券，积分,红包
        }
        //退款(加库存)
        if ($order['status'] == 4 && $org_status != 3)
        {
            ...
            St_SMSService::send_product_order_msg(NoticeCommon::PRODUCT_ORDER_REFUND_MSGTAG, $order);
            St_EmailService::send_product_order_email(NoticeCommon::PRODUCT_ORDER_REFUND_MSGTAG, $order);
            self::discounts_cancel($order['ordersn'],$order);//处理优惠券，积分,红包
        }
        ...
        if ($order['status'] == 6)
        {
            St_SMSService::send_product_order_msg(NoticeCommon::PRODUCT_ORDER_REVIEWING_MSGTAG, $order);
            St_EmailService::send_product_order_email(NoticeCommon::PRODUCT_ORDER_REVIEWING_MSGTAG, $order);
        }
        //由取消变为处理中(减库存)
        ...
        //订单写入供应商佣金
        if(in_array($order['status'],array(2,5))&&St_Functions::is_normal_app_install('supplierfinancemanage'))
        {
            Model_Supplier_Brokerage::add_brokerage($order['ordersn'],$order);
        }
        //完成交易返积分
        if($order['status'] == 5)
        {
            //修改完成时间
            DB::update('member_order')->set(array('finishtime'=>time()))->where('ordersn','=',$order['ordersn'])->execute();
            Model_Member_Jifen::refund($order['id']);//预订成功返积分
            //处理红包信息
            if (St_Functions::is_normal_app_install('red_envelope'))
            {
                Model_Order_Envelope::add_order_envelope($order['ordersn'],$order);
     ...
     * @param $order 订单
     */
    public static function discounts_cancel($ordersn,$order=null)
    {
        if (St_Functions::is_normal_app_install('coupon'))
        {
        ...
        if (St_Functions::is_normal_app_install('red_envelope'))
        {
            Model_Order_Envelope::cancel_order_back($ordersn);
        }
    ...
    {
        $result =  DB::delete('member_order')->where('id', '=', $orderid)->execute();
        DB::delete('member_order_compute')->where('order_id','=',$orderid)->execute();
        return $result;
    }
    ...
    public static function back_order_paysuccess($orderid, $paySource = '后台', $params = null)
    {
        ...
        DB::update('member_order')->set(array('paytime'=>time()))->where('id','=',$orderid)->execute();//修改支付时间
    ...
    {
        $w = "addtime>=$time_arr[0] and addtime<=($time_arr[1]+86399) and typeid='$typeid'";
        ...
        foreach ($arr as $row)
        {
            ...
            $table .= "<td style='vnd.ms-excel.numberformat:@'>"."{$row['eticketno']}</td>";
            $table .= "</tr>";
        }
     ...
     * @param $order
     */
    public static function update_order_jifen($order)
    {
        $new_jifen_book = St_Product::calculate_jifenbook($order['jifenbook_id'], $order['typeid'], $order['ordersn']);

        $data = array(
            'jifenbook' => $new_jifen_book
        );
        DB::update('member_order')->set($data)->where('id','=',$order['id'])->execute();
